...
function verilog_systemverilog#PopFromVariable(variable, value)
  let list = split(a:variable, ',')
  if (count(list, a:value) != 0)
    return join(filter(list, "v:val !=# a:value"), ',')
  endif
  return a:variable
endfunction
" }}}
...
function verilog_systemverilog#CompleteCommand(lead, command, cursor)
  ...
  call verilog_systemverilog#Verbose('Current leading values list: [' . join(lead_list, ',') . '] (length = ' . len(lead_list) . ')')
  if (a:lead =~ ',$')
  ...
  for item in initial_lead
    call remove(valid_completions, item)
  endfor
